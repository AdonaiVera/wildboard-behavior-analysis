...
class ModelTrainer:
    ...
    def fine_tune_model(self, ds_train, ds_valid, ds_test, epochs=20, batch_size=16):
        pytorch_dataset_train = PyTorchDetectionDataset(
            ds_train, self.processor, transform=self.train_augmentation_and_transform)
        pytorch_dataset_valid = PyTorchDetectionDataset(
            ds_valid, self.processor, transform=self.valid_transform)
        pytorch_dataset_test = PyTorchDetectionDataset(
            ds_test, self.processor, transform=self.valid_transform)
